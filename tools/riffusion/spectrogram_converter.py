...
class SpectrogramConverter:
    ...
    def __init__(self, params: SpectrogramParams, device: str = "cuda"):
        self.p = params  # pylint: disable=invalid-name
        ...
        self.spectrogram_func = torchaudio.transforms.Spectrogram(
            ...
            power=None,  # Assuming you want to keep the amplitude-only behavior
            normalized=False,
            wkwargs=None,
            center=True,
            pad_mode="reflect",
        ...
        # MelScale instantiation (adjust if your SpectrogramParams requires different norm or mel_scale)
        self.mel_scaler = torchaudio.transforms.MelScale(
            ...
            norm=params.mel_scale_norm,  # Ensure this matches your needs
            mel_scale=params.mel_scale_type,  # Ensure this matches your needs
        ).to(self.device)

        # https://pytorch.org/audio/stable/generated/torchaudio.transforms.InverseMelScale.html
        # InverseMelScale instantiation (adjust if your SpectrogramParams requires different norm or mel_scale)
        self.inverse_mel_scaler = torchaudio.transforms.InverseMelScale(
            ...
            norm=params.mel_scale_norm,  # Ensure this matches your needs
            mel_scale=params.mel_scale_type,  # Ensure this matches your needs
            driver="gels",  # Default value, adjust if needed
        ).to(self.device)
    ...
    def mel_amplitudes_from_waveform(
        ...
        spectrogram_complex = self.spectrogram_func(  # pylint: disable=not-callable
            waveform
        )
        ...
        return self.mel_scaler(amplitudes)  # pylint: disable=not-callable
    ...
    def waveform_from_mel_amplitudes(
        ...
        amplitudes_linear = self.inverse_mel_scaler(  # pylint: disable=not-callable
            amplitudes_mel
        )

        # Run the approximate algorithm to compute the phase and recover the waveform
        return self.inverse_spectrogram_func(  # pylint: disable=not-callable
            amplitudes_linear
        )
